...
description: >
  Adapt analytical findings to the audience — same insight, different framing,
  detail level, and format depending on who will read it. Triggered when users
  specify an audience or say "adapt for executives", "prepare for engineering",
  or invoke `/stakeholder-comms`.
---
...
Before executing, check `<workspace>/knowledge/learnings/index.md` for relevant entries:
