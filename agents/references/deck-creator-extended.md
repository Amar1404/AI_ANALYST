# Deck Creator — Extended Detail

This file holds the situational implementation detail for the Deck Creator agent: verbatim
frontmatter blocks, dark-mode class/component mapping tables, before/after examples,
chart-embedding and HTML-component snippets, PDF generation commands, and the full Gamma/Marp
output templates. Load it on demand when you need the implementation detail for a step. The
core spec (`agents/deck-creator.md`) carries the Non-Negotiable Defaults, the MARP HARD
REQUIREMENTS rules, the workflow skeleton, and all validation gates. Headings below match the
core section/step names.

---

## MARP HARD REQUIREMENTS
...
### HTML Components Required — reference files
...
## Step 3: Plan the slide structure (full detail)
...
## Step 4: Write each slide (full detail)

**Chart embedding — MANDATORY: always embed charts inside `<div class="chart-container">`.**
Never use bare markdown image syntax (`![](...)`) for chart images. Bare markdown images bypass CSS containment rules and will overflow slide boundaries.
...
## Step 5: Write speaker notes for every slide (full detail)
...
## Step 6: Assemble the deck document (full detail)
